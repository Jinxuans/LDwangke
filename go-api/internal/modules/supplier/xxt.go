...
var (
	xxtPhoneLoginURLTpl   = "https://passport2-api.chaoxing.com/v11/loginregister?cx_xxt_passport=json"
	xxtSchoolSearchURLTpl = "https://passport2-api.chaoxing.com/org/searchUnis?filter=%s&product=1&type="
	xxtIDLoginURLTpl      = "https://passport2-api.chaoxing.com/v6/idNumberLogin?fid=%s&idNumber=%s"
	xxtCourseURL          = "https://mooc1-api.chaoxing.com/mycourse/"
	xxtHTTPClientFactory  = func() *http.Client {
		return &http.Client{Timeout: 30 * time.Second}
	}
)

func xxtCallQuery(account, password, school string) (map[string]interface{}, error) {
	client := xxtHTTPClientFactory()
	...
	if isPhone {
		...
		resp, err := client.PostForm(xxtPhoneLoginURLTpl, form)
		if err != nil {
		...
		if status, _ := loginResp["status"].(bool); !status {
			return map[string]interface{}{"code": -1, "msg": "信息错误或者重试"}, nil
		}
		cookie = extractCookies(resp)
	} else {
		schoolURL := fmt.Sprintf(xxtSchoolSearchURLTpl, url.QueryEscape(school))
		...
		_ = json.Unmarshal(body, &schoolResp)

		froms, _ := schoolResp["froms"].([]interface{})
		if len(froms) == 0 {
			return map[string]interface{}{"code": -1, "msg": "未找到学校信息"}, nil
		}
		firstSchool := froms[0].(map[string]interface{})
		fid := toString(firstSchool["schoolid"])

		loginURL := fmt.Sprintf(xxtIDLoginURLTpl, fid, account)
		form := url.Values{}
		...
		var loginResp map[string]interface{}
		_ = json.Unmarshal(body2, &loginResp)
		if status, _ := loginResp["status"].(bool); !status {
			return map[string]interface{}{"code": -1, "msg": "信息错误或者重试"}, nil
		}
	...
	req, _ := http.NewRequest("GET", xxtCourseURL, nil)
	...
	_ = json.Unmarshal(courseResult, &courseResp)
	...
	courses := make([]map[string]interface{}, 0, len(channelList))
	for _, ch := range channelList {
		...
		firstCourse, ok := dataArr[0].(map[string]interface{})
		if !ok {
			continue
		}

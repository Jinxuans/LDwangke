...
func adminMoneyLogList(page, limit int, uid string, logType string) ([]map[string]interface{}, int64, error) {
	if page <= 0 {
		page = 1
	}
	if limit <= 0 {
...
func dashboardStats() (map[string]interface{}, error) {
	...
	stats["trend"] = getWeekTrend()
	stats["recent_orders"] = getRecentOrders()
	stats["status_distribution"] = getOrderStatusDistribution()
	stats["top_users"] = getTopUsers(7)

	return stats, nil
}

func getTopUsers(days int) []map[string]interface{} {
	rows, err := database.DB.Query(
...
func getWeekTrend() []map[string]interface{} {
	rows, err := database.DB.Query(
...
func getRecentOrders() []map[string]interface{} {
	rows, err := database.DB.Query(
...
func statsReport(days int) (map[string]interface{}, error) {
	if days <= 0 {
		days = 30
	}
...
func supplierRanking() ([]SupplierRankItem, error) {
...
func agentProductRanking(uid int, timeType string, limit int) ([]AgentProductRankItem, error) {
	if uid <= 0 {

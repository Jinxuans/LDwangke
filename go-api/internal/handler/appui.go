...
func AppuiGetPrice(c *gin.Context) {
	...
	svc := service.NewAppuiService()
	price, err := svc.GetPrice(uid, req.PID, req.Days)
	if err != nil {
...
func AppuiOrderList(c *gin.Context) {
	...
	svc := service.NewAppuiService()
	orders, total, err := svc.ListOrders(uid, isAdmin, page, limit, searchType, keyword)
	if err != nil {
...
func AppuiEditOrder(c *gin.Context) {
	...
	svc := service.NewAppuiService()
	if err := svc.EditOrder(uid, isAdmin, req.Form); err != nil {
		response.BusinessError(c, -1, err.Error())
		return
	}
...
func AppuiRenewOrder(c *gin.Context) {
	...
	svc := service.NewAppuiService()
	msg, err := svc.RenewOrder(uid, isAdmin, req.ID, req.Days)
	if err != nil {
		response.BusinessError(c, -1, err.Error())
		return
	}

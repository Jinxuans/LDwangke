...
// 参数和返回格式与PHP系统完全一致
func OpenAPICompat(c *gin.Context) {
	act := c.Query("act")
	if act == "" {
		act = c.PostForm("act")
	}
...
func compatQuery(c *gin.Context) {
	...
	svc := service.NewOpenAPIService()
	if err := svc.CheckQueryBalance(money); err != nil {
		c.JSON(200, gin.H{"code": -2, "msg": err.Error()})
		return
	}

	result, err := svc.QueryCourse(uid, money, cid, userinfo)
	if err != nil {
...
func compatAdd(c *gin.Context) {
	...
	svc := service.NewOpenAPIService()
	if err := svc.CheckOrderBalance(money); err != nil {
		c.JSON(200, gin.H{"code": -2, "msg": err.Error()})
		return
	}
	...
	result, err := svc.AddOrder(uid, money, req)
	if err != nil {
...
func compatChadan(c *gin.Context) {
	...
	svc := service.NewOpenAPIService()
	list, err := svc.Chadan(username, oid)
	if err != nil {
		c.JSON(200, gin.H{"code": -1, "msg": err.Error()})
		return
	}
...
func compatUp(c *gin.Context) {
	...
	_, err := orderSvc.SyncOrderProgress([]int{oid})
	if err != nil {
		c.JSON(200, gin.H{"code": -1, "msg": err.Error()})
		return
	}
...
func compatBindPushUID(c *gin.Context) {
	...
	svc := service.NewOpenAPIService()
	if err := svc.BindPushUID(orderID, pushUID); err != nil {
...
func compatBindPushEmail(c *gin.Context) {
	...
	svc := service.NewOpenAPIService()
	if err := svc.BindPushEmail(orderID, account, pushEmail); err != nil {
		c.JSON(200, gin.H{"code": 0, "msg": err.Error()})
		return
	}
	c.JSON(200, gin.H{"code": 1, "msg": "操作成功"})
...
func compatBindShowDocPush(c *gin.Context) {
	...
	svc := service.NewOpenAPIService()
	if err := svc.BindShowDocPush(orderID, account, showdocURL); err != nil {
		c.JSON(200, gin.H{"code": 0, "msg": err.Error()})
		return
	}
	c.JSON(200, gin.H{"code": 1, "msg": "操作成功"})

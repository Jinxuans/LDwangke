...
func verifyBridgeSign(c *gin.Context) bool {
	...
	secret := config.Global.Server.BridgeSecret
...
func BridgeMoneyChange(c *gin.Context) {
	...
	fmt.Printf("[PHP-Bridge] 余额变动: uid=%d amount=%.2f reason=%s\n", uid, amount, reason)
...
func BridgeCreateOrder(c *gin.Context) {
	...
	fmt.Printf("[PHP-Bridge] 创建订单: oid=%d uid=%d cid=%d user=%s kcname=%s\n", oid, uid, cid, userName, kcname)

...
func Login(c *gin.Context) {
	...
	// 将 refreshToken 写入 HttpOnly cookie（与 Vben mock 行为一致）
	service.SetRefreshTokenCookie(c.Writer, refreshToken, config.Global.JWT.RefreshTTL)
...
func RefreshToken(c *gin.Context) {
	// Vben 前端从 cookie 读取 refresh_token
	...
	// 更新 cookie
	service.SetRefreshTokenCookie(c.Writer, newRefreshToken, config.Global.JWT.RefreshTTL)

	// Vben mock 返回裸 accessToken 字符串

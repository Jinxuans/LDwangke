  ...
  <div class="app-content-body">
    <div class="wrapper-md control" id="pangu">
      ...
      <el-card class="box-card">
        <el-alert title="下单前请先完成授权,授权地址：https://sdauth.666hhh.sbs/" type="warning"></el-alert>
        <div style="height: 10px"></div>
        <el-button type="primary" @click="()=>{addDialog = true}" size="small" plain>提交订单</el-button>
        <el-button type="success" @click="()=>{window.open('https://sdauth.666hhh.sbs/', '_blank')}" size="small" plain>授权链接</el-button>
        ...
        <el-table :data="tableData" v-loading="tableLoading" element-loading-text="载入数据中" ref="multipleTable"
          ...
          <?php } ?>
          <el-table-column prop="uid" label="手机号" width="100" align="center">
          </el-table-column>
          ...
          <el-table-column label="跑步类型" width="100" align="center">
            <template slot-scope="{ row }">
              <el-tag v-if="row.run_type == 1" type="primary">阳光跑</el-tag>
              <el-tag v-else-if="row.run_type == 2" type="success">自由跑</el-tag>
            </template>
          </el-table-column>
      ...
      <el-dialog id="addDialog" title="添加订单" :visible.sync="addDialog" :before-close="handleAddClose"
        ...
        <el-form label-width="90px">
          <el-form-item label="闪动手机号">
            <el-input v-model="userInfo.uid" :disabled="userInfo.disable" size="small" placeholder="请输入闪动手机号"></el-input>
          </el-form-item>
          ...
          <el-form-item label="跑步计划">
            <el-select v-model="addForm.runPlanCode" @change="handleRunPlanChange" placeholder="请选择跑步计划" size="small">
              <el-option v-for="item in runPlanOptions" :key="item.runPlanCode" :label="item.isSemester ? `--- ${item.semesterName} ---` : item.runPlanName" :disabled="item.disabled" :value="item.runPlanCode">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="跑步区域">
            <el-select v-model="addForm.fenceCode" placeholder="请选择跑步区域" size="small">
              <el-option v-for="item in runZoneOptions" :key="item.fenceCode" :label="item.fenceName" :value="item.fenceCode">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="跑步类型">
            <el-radio-group v-model="addForm.runType" size="small">
              <el-radio-button :label="1">阳光跑</el-radio-button>
              <el-radio-button :label="2">自由跑</el-radio-button>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item label="跑步距离">
            <el-input-number v-model.number="addForm.runMeter" :precision="1" :step="0.1" :min="distanceMin"
              :max="distanceMax" label="跑步距离" size="small"></el-input-number>
          </el-form-item>
...
<script src="/pangu/static/pgsdxy.js"></script>
  ...
  <div class="app-content-body">
    <div class="wrapper-md control" id="pangu">
      ...
      <el-card class="box-card">
        <el-alert title="下单前请先完成授权,授权地址：https://keepauth.666hhh.sbs/" type="warning"></el-alert>
        <div style="height: 10px"></div>
        <el-button type="primary" @click="()=>{addDialog = true}" size="small" plain>提交订单</el-button>
        <el-button type="success" @click="()=>{window.open('https://keepauth.666hhh.sbs/', '_blank')}" size="small" plain>授权链接</el-button>
        <el-button type="warning" size="small">每公里价格：{{price}} / 次</el-button>
        ...
        <el-table :data="tableData" v-loading="tableLoading" element-loading-text="载入数据中" ref="multipleTable"
          ...
          <el-table-column label="跑步类型" width="100" align="center">
            <template slot-scope="{ row }">
              <el-tag v-if="row.run_type == 'running'" type="success">跑步</el-tag>
              <el-tag v-if="row.run_type == 'cycling'" type="primary">骑行</el-tag>
              <el-tag v-if="row.run_type == 'hiking'" type="warning">步行</el-tag>
            </template>
          </el-table-column>
      ...
      <el-dialog id="addDialog" title="添加订单" :visible.sync="addDialog" :before-close="handleAddClose"
        ...
        <el-form label-width="90px">
          ...
          <el-form-item label="跑步类型">
            <el-radio-group v-model="addForm.sportType" size="small">
              <el-radio-button label="running">跑步</el-radio-button>
              <el-radio-button label="cycling" disabled="">骑行</el-radio-button>
              <el-radio-button label="hiking" disabled="">步行</el-radio-button>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item label="跑步距离">
            <el-input-number v-model.number="addForm.sportKMeter" :precision="1" :step="0.1" :min="distanceMin"
              :max="10" label="跑步距离" size="small"></el-input-number>
          </el-form-item>
          <el-form-item label="搜索方式">
            <el-radio v-model="customZone.type" :label="1">平台内缓存路线</el-radio>
            <el-radio v-model="customZone.type" :label="2" disabled="">实时地图</el-radio>
          </el-form-item>
          <el-form-item label="跑步路线">
            <el-select
              size="small"
              v-model="addForm.raceId"
              filterable
              remote
              placeholder="请输入关键词"
              :remote-method="zoneRemoteMethod"
              :loading="zoneRemoteLoading">
              <el-option
          ...
          <el-form-item label="跑步周期">
            <el-checkbox v-for="item in runWeekOptions" v-model="addForm.sportWeekDay" :label="item.value">{{ item.label }}</el-checkbox>
          </el-form-item>
          <el-form-item label="跑步配速">
            ...
            <el-slider class="mx-3" v-model="addForm.sportPaceList" :disabled="!customSpeed.flag" range show-stops
      ...
      <el-dialog title="订单执行日志" :visible.sync="logDialog" fullscreen :before-close="handleLogClose">
        <el-table :data="logForm.list" v-loading="logLoading" border size="small">
          ...
          <el-table-column align="center" label="跑步类型" width="100">
            <template slot-scope="{ row }">
              <el-tag v-if="row.sport_type == 'running'" type="success">跑步</el-tag>
              <el-tag v-if="row.sport_type == 'cycling'" type="primary">骑行</el-tag>
              <el-tag v-if="row.sport_type == 'hiking'" type="warning">步行</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column align="center" property="sport_km" label="距离" width="80"></el-table-column>
...
<script src="/pangu/static/pgkeep.js"></script>
  ...
  <div class="app-content-body">
    <div class="wrapper-md control" id="pangu">
      ...
      <el-card class="box-card">
        <el-alert title="下单前请先完成授权,授权地址：https://tsn.666hhh.sbs/" type="warning"></el-alert>
        <div style="height: 10px"></div>
        <el-button type="primary" @click="()=>{addDialog = true}" size="small" plain>提交订单</el-button>
        <el-button type="success" @click="()=>{window.open('https://tsn.666hhh.sbs/', '_blank')}" size="small" plain>授权链接</el-button>
        ...
        <el-table :data="tableData" v-loading="tableLoading" element-loading-text="载入数据中" ref="multipleTable"
          ...
          <el-table-column label="跑步类型" width="100" align="center">
            <template slot-scope="{ row }">
              <el-tag v-if="row.run_type == 1" type="primary">晨跑</el-tag>
              <el-tag v-if="row.run_type == 2" type="success">阳光跑</el-tag>
            </template>
          </el-table-column>
      ...
      <el-dialog id="addDialog" title="添加订单" :visible.sync="addDialog" :before-close="handleAddClose"
        ...
        <el-form label-width="90px">
          ...
          <el-form-item label="操作">
            <el-button type="primary" @click="getRule" size="mini">获取缓存规则</el-button>
          </el-form-item>
          <el-form-item label="温馨提示">
            <span style="color: red">体适能不能获取到详细的规则，请按照学校规则设置，部分学校不区分跑区，这种情况请关闭自定义跑区</span>
          </el-form-item>
          ...
          <el-form-item label="自定义跑区">
            <el-switch v-model="customRunZone.flag">
            </el-switch>
          </el-form-item>
          <el-form-item label="跑步区域" v-show="customRunZone.flag">
            <el-select v-model="customRunZone.id" placeholder="请选择跑步区域" size="small">
              <el-option v-for="item in runZoneOptions" :key="item.id" :label="item.formatted_address" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="跑步类型">
            <el-radio-group v-model="addForm.runType" size="small">
              <el-radio-button :label="1" :disabled="userInfo.canRunTypeList.includes(1) == false">晨跑</el-radio-button>
              <el-radio-button :label="2" :disabled="userInfo.canRunTypeList.includes(2) == false">阳光跑</el-radio-button>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item label="跑步距离">
            <el-input-number v-model.number="addForm.runMeter" :precision="1" :step="0.1" :min="distanceMin"
              :max="5" label="跑步距离" size="small"></el-input-number>
          </el-form-item>
          ...
          <el-form-item label="跑步耗时">
            <div style="display: flex;align-items: center;justify-content: space-between;">
              <div>配速：{{ currentSpeed }}，耗时：{{ currentUsedTime }}</div>
              <el-switch v-model="customUsedTime.flag" active-text="自定义">
              </el-switch>
            </div>
            <el-slider class="mx-3" :disabled="!customUsedTime.flag" v-model="customUsedTime.range" range show-stops :min="customUsedTime.min" :max="customUsedTime.max">
            </el-slider>
            </el-slider>
          </el-form-item>
...
<script src="/pangu/static/pgtsn.js"></script>
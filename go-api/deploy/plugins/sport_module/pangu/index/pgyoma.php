  ...
  <div class="app-content-body">
    <div class="wrapper-md control" id="pangu">
      ...
      <el-card class="box-card">
        <el-alert title="下单前请先完成授权,授权地址：https://ymauth.666hhh.sbs/" type="error"></el-alert>
        <div style="height: 10px"></div>
        <el-button type="primary" @click="()=>{addDialog = true}" size="small" plain>提交订单</el-button>
        <el-button type="success" @click="()=>{window.open('https://ymauth.666hhh.sbs/', '_blank')}" size="small" plain>授权链接</el-button>
        ...
        <el-table :data="tableData" v-loading="tableLoading" element-loading-text="载入数据中" ref="multipleTable"
          ...
          <?php } ?>
          <el-table-column prop="uid" label="账号UID" width="150" align="center">
          </el-table-column>
          <el-table-column prop="residue_num" label="剩余次数" width="100" align="center">
          ...
          <el-table-column label="跑步距离" width="100" align="center">
            <template slot-scope="{ row }">
              {{ row.run_meter }} KM
            </template>
          </el-table-column>
          <el-table-column label="跑步类型" width="100" align="center">
            <template slot-scope="{ row }">
              <el-tag type="success" v-if="row.run_type == 4">健康晨跑</el-tag>
              <el-tag type="primary" v-else-if="row.run_type == 5">阳光长跑</el-tag>
              <el-tag type="danger" v-else-if="row.run_type == 6">重修跑</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="mark_text" label="订单备注" min-width="100" align="center">
          </el-table-column>
          <el-table-column prop="created_at" label="下单时间" width="120" align="center">
          </el-table-column>
      ...
      <el-dialog id="addDialog" title="添加订单" :visible.sync="addDialog" :before-close="handleAddClose"
        ...
        <el-form label-width="90px">
          ...
          <el-form-item label="跑步区域">
            <el-select v-model="addForm.railId" placeholder="请选择跑步类型" size="small">
              <el-option v-for="item in runZoneOptions" :key="item.railId" :label="item.districtName + '-' + item.railName" :value="item.railId">
                <span style="float: left">{{ item.districtName + "-" + item.railName }}</span>
                <span style="float: right">
                  <el-tag v-if="item.havePath" type="success" size="mini">有路线</el-tag>
                  <el-tag v-else type="danger" size="mini">无路线</el-tag>
                </span>
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="跑步类型">
            <el-radio-group v-model="addForm.runType" @change="changeRunType" size="small">
              <el-radio-button :label="4" :disabled="userInfo.runTypeStatus.morningRun">健康晨跑</el-radio-button>
              <el-radio-button :label="5" :disabled="userInfo.runTypeStatus.sunRun">阳光长跑</el-radio-button>
              <el-radio-button :label="6" :disabled="userInfo.runTypeStatus.rebuildRun">重修跑</el-radio-button>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item label="跑步时间">
            ...
            <el-select v-show="!customTime.flag" v-model="addForm.startTimeRange" placeholder="请选择跑步时间段" size="small">
              <el-option v-for="item in startTimeRangeOptions" :key="item" :label="item" :value="item">
              </el-option>
            </el-select>
            <!-- 预选时间段 - 结束 -->
            <!-- 自定义时间段 - 开始 -->
          ...
          <el-form-item label="跑步距离">
            <el-input-number v-model.number="addForm.runKMeter" :precision="1" :step="0.1" :min="distanceMin"
          ...
          <el-form-item label="跑步配速">
            ...
            <el-slider class="mx-3" v-model="addForm.runPaceList" :disabled="!customSpeed.flag" range show-stops
...
<script src="/pangu/static/pgyoma.js"></script>
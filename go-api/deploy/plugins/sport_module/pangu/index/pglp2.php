  ...
  <div class="app-content-body">
    <div class="wrapper-md control" id="pangu">
      ...
      <el-card class="box-card">
        <el-alert title="下单前请先完成授权,授权地址：https://abels.gitbook.io/wpauth" type="error"></el-alert>
        <div style="height: 10px"></div>
        <el-button type="primary" @click="()=>{addDialog = true}" size="small" plain>提交订单</el-button>
        <el-button type="success" @click="()=>{window.open('https://abels.gitbook.io/wpauth', '_blank')}" size="small" plain>授权链接</el-button>
        ...
        <el-table :data="tableData" v-loading="tableLoading" element-loading-text="载入数据中" ref="multipleTable"
          ...
          <?php } ?>
          <el-table-column prop="uid" label="账号UID" width="100" align="center">
          </el-table-column>
          ...
          <el-table-column label="跑步类型" width="100" align="center">
            <template slot-scope="{ row }">
              <el-tag v-if="row.run_type == 0" type="success">自由跑</el-tag>
              <el-tag v-if="row.run_type == 1" type="primary">乐跑</el-tag>
              <el-tag v-if="row.run_type == 2" type="danger">线下乐跑</el-tag>
              <el-tag v-if="row.run_type == 3" type="warning">无感抓拍</el-tag>
            </template>
          </el-table-column>
            ...
            <template slot-scope="scope">
              <el-dropdown @command="handleCommand">
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item :command="{item:scope.row,type:'edit'}">编辑订单</el-dropdown-item>
      ...
      <!-- 列表 - 结束 -->
      <!--添加弹窗 - 开始-->
      <el-dialog id="addDialog" :title="isEdit?'编辑订单':'添加订单'" :visible.sync="addDialog" :before-close="handleAddClose"
        size="small" top="20px">
        <el-form label-width="90px">
          <el-form-item label="UID">
            <el-input v-model="userInfo.uid" :disabled="userInfo.disable || isEdit" size="small" placeholder="请输入UID"></el-input>
          </el-form-item>
          <el-form-item label="操作">
            <el-button type="primary" @click="getCacheRule" size="mini">获取缓存规则</el-button>
            <el-button type="primary" @click="getRealTimeRule" size="mini">获取实时规则</el-button>
          </el-form-item>
          ...
          <el-form-item label="跑步区域">
            <el-select v-model="addForm.runZoneId" @change="changeRunZone" placeholder="请选择跑步区域" size="small">
              <el-option v-for="item in runZoneOptions" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="温馨提示" v-show="addForm.runType == 2 || addForm.runType == 3">
            <span style="color: red" v-show="addForm.runType == 2">该跑区为人脸打卡跑区，单次价格为{{ (price * 2).toFixed(2) }}</span>
            <span style="color: red" v-show="addForm.runType == 3">该跑区为无感抓拍跑区，单次价格为{{ (price * 1.61).toFixed(2) }}</span>
          </el-form-item>
          <el-form-item label="跑步类型">
            <el-radio-group v-model="addForm.runType" size="small">
              <el-radio-button :label="0">自由跑</el-radio-button>
              <!-- 同属乐跑系列 -->
              <el-radio-button :label="2" :disabled="userInfo.justFreeRun" v-if="addForm.runType == 2">线下乐跑</el-radio-button>
              <el-radio-button :label="3" :disabled="userInfo.justFreeRun" v-else-if="addForm.runType == 3">无感抓拍</el-radio-button>
              <el-radio-button :label="1" :disabled="userInfo.justFreeRun" v-else>乐跑</el-radio-button>
              <!-- 同属乐跑系列 -->
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item label="跑步时间">
            <!-- 预选时间段 - 开始 -->
            <el-select v-show="!customTime.flag" v-model="addForm.startTimeRange" placeholder="请选择跑步时间段" size="small">
              <el-option v-for="item in startTimeRangeOptions" :key="item.runRangeTime" :label="item.runRangeTime"
                :value="item.runRangeTime">
              </el-option>
            </el-select>
            <!-- 预选时间段 - 结束 -->
            <!-- 自定义时间段 - 开始 -->
          ...
          <el-form-item label="跑步配速">
            ...
            <el-slider class="mx-3" v-model="addForm.runSpeed" :disabled="!customSpeed.flag" range show-stops
              :min="customSpeed.min" :max="customSpeed.max">
            </el-slider>
          </el-form-item>
          <el-form-item label="下单次数">
            <el-input-number :disabled="isEdit" v-model.number="addForm.orderNum" :step="1" :min="1" :max="100" label="下单次数"
              size="small"></el-input-number>
            <span style="color: red;margin-left: 12px;font-size: 18px;font-weight: bold;" v-show="!isEdit">￥ {{ total }}</span>
          </el-form-item>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="submitForm" :loading="addLoading">{{ isEdit?'确认修改':'确认下单' }}</el-button>
        </div>
      </el-dialog>
      <!--添加弹窗 - 结束-->
      <!--增加次数弹窗 - 开始-->
...
<script src="/pangu/static/pglp2.js"></script>
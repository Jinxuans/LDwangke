...
CREATE TABLE IF NOT EXISTS `qingka_wangke_sync_config` (
  ...
  `clone_category` tinyint(1) NOT NULL DEFAULT 0 COMMENT '克隆时同步分类',
  `skip_categories` text COMMENT '跳过分类ID JSON数组，如["3","5"]',
  `name_replace` text COMMENT '名称替换规则JSON，如{"旧词":"新词"}',
  `secret_price_rate` decimal(10,4) NOT NULL DEFAULT 0 COMMENT '同步保密价倍率/上游成本展示倍率，0表示不写入商品保密价',
  `auto_sync_enabled` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否开启自动定时同步',
  `auto_sync_interval` int(11) NOT NULL DEFAULT 30 COMMENT '自动同步间隔（分钟）',
...
CREATE TABLE IF NOT EXISTS `qingka_platform_config` (
  ...
  `query_act` varchar(50) NOT NULL DEFAULT '' COMMENT '查课专用驱动标识',
  ...
  `progress_path` varchar(200) NOT NULL DEFAULT '/api.php?act=chadan2' COMMENT '进度路径',

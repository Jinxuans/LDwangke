...
- Windows File I/O Fix: 1. Use double backslashes \\ for all paths; 2. Support all valid paths (relative/absolute), wrap space-included paths with double quotes; 3. Exact same path for read/write on the same file; 4. Split large content into multiple append writes; 5. Re-read first then retry on write failure.
...
为避免文件写入时出现参数错误、JSON 解析失败等问题，所有文件写入操作必须遵守以下规则。
...
#### 2. 参数完整性要求
- 强制 AI 输出的写入指令中，`filePath` 为项目内绝对路径字符串
- 示例：`/www/wwwroot/29.colnt.com/bt_plugin/qingka_manager/qingka_manager_main.py`
- `content` 为完整代码字符串
- **禁止**出现 `undefined`/空值

#### 3. JSON 语法校验
- 要求 AI 输出写入指令前，自检 JSON 语法完整性
- 重点检查 `[]`/`{}` 配对，无括号缺失
- 明确告知校验结果

#### 4. 先纯代码后指令
- AI 需先输出纯文本格式的代码（无 JSON 封装）
- 确认代码无误后，再输出 OpenCode 合规的写入指令

### 二、操作执行规则（避免指令截断/异常）

#### 1. 小步快跑原则
- 放弃大文件全量重写
- 改为"按功能点增量修改"（如仅修改"日志清空函数""数据库连接逻辑"）
- 单次修改代码量不超过 500 行

#### 2. 上下文控制
- 若文件过大（超过 1000 行），要求 AI 按模块/函数拆分输出代码
- 避免耗尽 Claude 上下文窗口导致指令截断

#### 3. 手动兜底机制
- 若 AI 生成的写入指令持续报错，直接放弃 AI 生成指令
- 改为"AI 输出纯代码 + 手动构造写入命令"

#### 4. 中断异常任务
- 遇到 `Preparing write...` 卡住或解析失败时，立即按 `esc` 终止进程
- 避免无效等待

### 三、环境配置规则（根除技术层面问题）

#### 1. 关闭批量操作
- 在 OpenCode 设置中，禁用 `Enable Batch File Operations`（批量文件操作）
- 强制插件仅支持单文件写入

#### 2. 检查中转 API
- 若使用自建 Claude 中转 API，取消响应体长度限制（如 `response_max_size`）
- 避免 API 截断 JSON 指令

#### 3. 路径合规校验
- 确保文件路径无特殊字符（空格、中文、特殊符号）
- 优先使用绝对路径，避免相对路径解析错误

### 四、校验验证规则（确保写入成功）

#### 1. 代码预校验
- 拿到 AI 输出的纯代码后，先本地验证语法
- Python 代码用 `python -m py_compile` 检查
- 再执行写入

#### 2. 指令预校验
- 执行写入指令前，手动检查 JSON 结构：
  - 必含字段：`filePath`（字符串）、`content`（字符串）
  - 语法完整性：所有 `{}`/`[]` 配对，无缺失/多余符号

#### 3. 权限前置检查
- 确认目标文件/目录有读写权限
- 如宝塔面板 `wwwroot` 目录权限为 `www:www`
- 避免写入后权限异常

### 核心执行优先级

1. **先遵守「指令约束规则」**，强制 AI 输出合规内容
2. **再执行「操作执行规则」**，避免上下文/批量操作导致的截断
3. **最后通过「校验验证规则」**，确保写入前无语法/权限问题
...
| 文档 | 位置 | 适用范围 |
|------|------|----------|
| **CLAUDE.md** | `29.colnt.com/CLAUDE.md` | 全局开发准则（所有开发任务） |
| **AGENTS.md** | `29.colnt.com/AGENTS.md` | 文件写入操作安全规则（已整合至 CLAUDE.md） |

### 使用方式

- **CLAUDE.md** 是唯一的开发规范主文档，所有开发任务必须遵循
- 文件写入操作安全规则已整合至 CLAUDE.md，全局适用
- AGENTS.md 保留作为快速参考，但规范以 CLAUDE.md 为准
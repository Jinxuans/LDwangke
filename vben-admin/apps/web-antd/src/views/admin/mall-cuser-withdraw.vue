...
const columns = [
  ...
  { title: '店铺', dataIndex: 'tid', key: 'tid', width: 80 },
...
async function loadData(page = 1) {
  ...
  try {
    const res = await requestClient.get<any>('/admin/mall-cuser-withdraw/requests', {
      params: {
        page: pagination.page,
        limit: pagination.limit,
        tid: filterTid.value || undefined,
...
async function submitReview() {
  ...
  try {
    await requestClient.post(`/admin/mall-cuser-withdraw/${currentRow.value.id}/review`, {
...
<template>
  <Page title="C端会员佣金提现审核" content-class="p-4">
    <Card>
      <div class="mb-4 flex flex-wrap items-center gap-3">
        <Space wrap>
          <Input v-model:value="filterTid" placeholder="店铺TID" allow-clear style="max-width: 120px; min-width: 90px" @pressEnter="() => loadData(1)" />
      ...
      <Spin :spinning="loading">
        <Table :data-source="list" :columns="columns" :pagination="false" row-key="id" size="small" bordered :scroll="{ x: 1320 }">
    ...
    <Modal v-model:open="reviewVisible" :title="reviewStatus === 1 ? '确认会员已线下打款' : '驳回会员提现申请'" :confirm-loading="reviewing" ok-text="提交" @ok="submitReview">

...
function openAdd() {
  ...
  editForm.value = {
    auth_type: 'uid_key',
    api_path_style: 'standard',
    success_codes: '0',
    query_act: 'get',
    order_act: 'add',
    progress_act: 'chadan2',
    ...
    log_method: 'POST',
    log_id_param: 'id',
    balance_act: 'getmoney',
    balance_money_field: 'money',
    balance_method: 'POST',
    balance_auth_type: '',
  };
...
async function handleSave() {
  if (!editForm.value.pt) {
    message.warning('请填写平台标识');
    return;
  }
...
async function parsePHP() {
  ...
  try {
    ...
    if (phpResult.value && phpResult.value.confidence >= 30) {
      ...
      message.warning('解析置信度较低，建议手动核对');
    }
  } catch (e: any) {
...
function applyPHPResult() {
  ...
  editForm.value = {
    pt: r.pt,
    name: r.name,
    auth_type: r.auth_type || 'uid_key',
    api_path_style: r.api_path_style || 'standard',
    success_codes: r.success_codes || '0',
    use_json: r.use_json,
    query_act: r.query_act || 'get',
    query_path: r.query_path || '',
    order_act: r.order_act || 'add',
    order_path: r.order_path || '',
    progress_act: r.progress_act || 'chadan2',
    ...
    log_act: r.log_act || 'xq',
    log_path: r.log_path || '',
    log_id_param: r.log_id_param || 'id',
    returns_yid: r.returns_yid,
    progress_no_yid: 'chadan',
    log_method: 'POST',
    balance_act: r.balance_act || 'getmoney',
    balance_path: r.balance_path || '',
    balance_money_field: r.balance_money_field || 'money',
    balance_method: 'POST',
    balance_auth_type: '',
...
async function runDetect() {
  if (!detectForm.value.url) {
    message.warning('请填写平台 URL');
    return;
  }
  ...
  try {
    ...
    if (detectResult.value?.success) {
      ...
      message.warning('未检测到可用接口，请检查 URL 和凭证');
    }
  } catch (e: any) {
...
function applyDetectResult() {
  ...
  editForm.value = {
    ...
    auth_type: cfg.auth_type || 'uid_key',
    api_path_style: cfg.api_path_style || 'standard',
    success_codes: cfg.success_codes || '0',
    use_json: cfg.use_json === 'true',
    query_act: cfg.query_act || '',
    query_path: cfg.query_path || '',
    order_act: cfg.order_act || '',
    order_path: cfg.order_path || '',
    progress_act: cfg.progress_act || '',
    progress_no_yid: cfg.progress_no_yid || '',
    ...
    change_pass_param: 'newPwd',
    change_pass_id_param: 'id',
    resubmit_path: cfg.resubmit_path || '',
    refresh_path: cfg.refresh_path || '',
    log_act: cfg.log_act || '',
    log_path: cfg.log_path || '',
    log_method: cfg.log_method || 'POST',
    log_id_param: 'id',
    balance_act: cfg.balance_act || '',
    balance_path: cfg.balance_path || '',
    balance_money_field: cfg.balance_money_field || 'money',
    balance_method: 'POST',
    balance_auth_type: cfg.auth_type === 'bearer_token' ? 'bearer_token' : '',
    report_path: cfg.report_path || '',
    get_report_path: cfg.get_report_path || '',
    report_param_style: cfg.report_param_style || '',
    report_auth_type: cfg.report_auth_type || '',
  };
  isEdit.value = false;
...
<template>
  <Page auto-content-height>
    <Card :bordered="false">
      <div class="mb-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <Typography.Title :level="5" style="margin: 0">平台接口配置</Typography.Title>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.dataIndex === 'auth_type'">
            <Tag :color="record.auth_type === 'uid_key' ? 'default' : 'orange'">
              {{ record.auth_type }}
            </Tag>
          </template>
          <template v-else-if="column.dataIndex === 'query_act'">
            <span>{{ record.query_act }}</span>
            <span v-if="record.query_path" class="ml-1 text-xs text-gray-400">
              {{ record.query_path }}
            </span>
          </template>
    ...
    <Modal
      ...
      :title="isEdit ? `编辑平台：${editForm.pt}` : '添加平台配置'"
        ...
        <Tabs size="small">
          ...
          <TabPane key="basic" tab="基本">
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="平台标识" required>
                <Input v-model:value="editForm.pt" :disabled="isEdit" placeholder="如 newplat" />
              </FormItem>
              <FormItem label="名称">
                <Input v-model:value="editForm.name" placeholder="如 新平台" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="认证方式">
            ...
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="成功码">
                <Input v-model:value="editForm.success_codes" placeholder="0,1,200" />
              </FormItem>
              <FormItem label="JSON请求">
                <Switch v-model:checked="editForm.use_json" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="需要代理">
                <Switch v-model:checked="editForm.need_proxy" />
              </FormItem>
              <FormItem label="返回YID">
                <Switch v-model:checked="editForm.returns_yid" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="额外参数">
                <Switch v-model:checked="editForm.extra_params" />
              </FormItem>
              <FormItem label="YID在数组">
                <Switch v-model:checked="editForm.yid_in_data_array" />
              </FormItem>
            </div>
          </TabPane>
          ...
          <TabPane key="query" tab="查课">
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="查课 act">
                <Input v-model:value="editForm.query_act" placeholder="get" />
              </FormItem>
              <FormItem label="查课路径">
                <Input v-model:value="editForm.query_path" placeholder="REST时填，如 /api/query" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="参数风格">
                <Input v-model:value="editForm.query_param_style" placeholder="standard" />
              </FormItem>
              <FormItem label="需要轮询">
                <Switch v-model:checked="editForm.query_polling" />
              </FormItem>
            </div>
          </TabPane>
          ...
          <TabPane key="order" tab="下单">
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="下单 act">
                <Input v-model:value="editForm.order_act" placeholder="add" />
              </FormItem>
              <FormItem label="下单路径">
                <Input v-model:value="editForm.order_path" placeholder="REST时填" />
              </FormItem>
            </div>
          </TabPane>
          ...
          <TabPane key="progress" tab="进度">
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="有YID act">
                <Input v-model:value="editForm.progress_act" placeholder="chadan2" />
              </FormItem>
              <FormItem label="无YID act">
                <Input v-model:value="editForm.progress_no_yid" placeholder="chadan" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="进度路径">
                <Input v-model:value="editForm.progress_path" placeholder="如 /api/search" />
              </FormItem>
              <FormItem label="请求方式">
            ...
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="用id参数">
                <Switch v-model:checked="editForm.use_id_param" />
              </FormItem>
              <FormItem label="用uuid参数">
                <Switch v-model:checked="editForm.use_uuid_param" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="总传username">
                <Switch v-model:checked="editForm.always_username" />
              </FormItem>
              <FormItem label="需要认证">
                <Switch v-model:checked="editForm.progress_needs_auth" />
              </FormItem>
            </div>
          </TabPane>
          ...
          <TabPane key="other" tab="暂停/改密/日志">
            ...
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="暂停 act">
                <Input v-model:value="editForm.pause_act" placeholder="zt" />
              </FormItem>
              <FormItem label="暂停路径">
                <Input v-model:value="editForm.pause_path" placeholder="如 /api/stop" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              ...
              <FormItem label="恢复路径">
                <Input v-model:value="editForm.resume_path" />
              </FormItem>
            </div>

            <Typography.Text type="secondary" class="mb-2 mt-3 block">改密码</Typography.Text>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="改密 act">
                <Input v-model:value="editForm.change_pass_act" placeholder="gaimi" />
              </FormItem>
              <FormItem label="改密路径">
                <Input v-model:value="editForm.change_pass_path" placeholder="如 /api/update" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="密码参数名">
                <Input v-model:value="editForm.change_pass_param" placeholder="newPwd" />
              </FormItem>
              <FormItem label="ID参数名">
                <Input v-model:value="editForm.change_pass_id_param" placeholder="id" />
              </FormItem>
            </div>

            <Typography.Text type="secondary" class="mb-2 mt-3 block">补单</Typography.Text>
            <FormItem label="补单路径">
              <Input v-model:value="editForm.resubmit_path" placeholder="如 /api/reset" />
            </FormItem>

            <Typography.Text type="secondary" class="mb-2 mt-3 block">日志</Typography.Text>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="日志 act">
                <Input v-model:value="editForm.log_act" placeholder="xq" />
              </FormItem>
              <FormItem label="日志路径">
                <Input v-model:value="editForm.log_path" placeholder="如 /log/" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="日志方式">
              ...
              <FormItem label="日志ID参数">
                <Input v-model:value="editForm.log_id_param" placeholder="id" />
              </FormItem>
            </div>
          </TabPane>
          ...
          <TabPane key="balance" tab="余额">
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="余额 act">
                <Input v-model:value="editForm.balance_act" placeholder="getmoney" />
              </FormItem>
              <FormItem label="余额路径">
                <Input v-model:value="editForm.balance_path" placeholder="如 /api/getinfo" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
          ...
          <TabPane key="report" tab="工单/刷新">
            ...
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="提交工单路径">
                <Input v-model:value="editForm.report_path" placeholder="如 /api/submitWork" />
              </FormItem>
              <FormItem label="查询工单路径">
                <Input v-model:value="editForm.get_report_path" placeholder="如 /api/queryWork" />
              </FormItem>
            </div>
            <div class="grid grid-cols-2 gap-x-4">
              <FormItem label="参数风格">
            ...
            <FormItem label="刷新路径">
              <Input v-model:value="editForm.refresh_path" placeholder="如 /api/refresh" />
            </FormItem>
          </TabPane>
        </Tabs>
      </Form>
    ...
    <!-- PHP 导入弹窗 -->
    <Modal
      v-model:open="phpVisible"
      title="从 PHP 代码导入平台配置"
      width="750px"
      :footer="null"
    >
      <Alert
        message="粘贴 PHP 的 if 代码块（如 ckjk.php / xdjk.php / xgjk.php 中的某个平台分支），系统会自动解析出配置"
        type="info"
        show-icon
        class="mb-3"
      />
      <Input.TextArea
        v-model:value="phpCode"
        :rows="12"
    ...
    <!-- 自动检测弹窗 -->
    <Modal
      v-model:open="detectVisible"
      title="自动检测平台接口"
      width="800px"
      :footer="null"
    >
      <Alert
        message="输入上游平台的 URL 和凭证，系统会自动探测支持的接口和参数格式"
        type="info"
        show-icon
        class="mb-3"
      />
      <Form :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }" size="small">
        <FormItem label="平台 URL" required>
          <Input v-model:value="detectForm.url" placeholder="http://xxx.com 或 xxx.com" />
        </FormItem>
        <div class="grid grid-cols-2 gap-x-4">
          <FormItem label="UID">
            <Input v-model:value="detectForm.uid" placeholder="上游账号" />
          </FormItem>
          <FormItem label="Key">
            <Input v-model:value="detectForm.key" placeholder="上游密钥" />
          </FormItem>
        </div>
        <div class="grid grid-cols-2 gap-x-4">
          <FormItem label="Token">
            <Input v-model:value="detectForm.token" placeholder="Bearer Token（可选）" />
          </FormItem>
          <FormItem label="Cookie">
            <Input v-model:value="detectForm.cookie" placeholder="Cookie（可选）" />
          </FormItem>
        </div>
        ...
        <div class="mt-2 rounded border p-4" :style="{ background: detectResult.success ? '#f6ffed' : '#fff2f0' }">
          <div class="mb-3 flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <Typography.Text strong style="font-size: 15px">
                {{ detectResult.success ? '检测成功' : '未检测到可用接口' }}
              </Typography.Text>
            </div>
          ...
          <template v-if="detectResult.success">
            <div class="mb-3 grid grid-cols-3 gap-3">
              <Card size="small" :bordered="true">
                ...
                <div class="font-medium">{{ detectResult.auth_type }}</div>
              </Card>
              <Card size="small" :bordered="true">
                <div class="text-xs text-gray-500">成功码</div>
                <div class="font-medium">{{ detectResult.success_code }}</div>
              </Card>
              <Card size="small" :bordered="true">
                <div class="text-xs text-gray-500">API 风格</div>
                <div class="font-medium">{{ detectResult.api_style }}</div>
              </Card>
            </div>
            <div class="mb-3 grid grid-cols-4 gap-3">
              <Card size="small" :bordered="true" :style="{ borderColor: detectResult.balance_ok ? '#52c41a' : '#d9d9d9' }">
                <div class="text-xs text-gray-500">余额</div>
                <div class="font-medium">{{ detectResult.balance_ok ? detectResult.balance_money : '不可用' }}</div>
              </Card>
              <Card size="small" :bordered="true" :style="{ borderColor: detectResult.query_ok ? '#52c41a' : '#d9d9d9' }">
                <div class="text-xs text-gray-500">查课</div>
                <div class="font-medium">{{ detectResult.query_ok ? '✓' : '✗' }}</div>
              </Card>
              <Card size="small" :bordered="true" :style="{ borderColor: detectResult.class_list_ok ? '#52c41a' : '#d9d9d9' }">
                <div class="text-xs text-gray-500">课程列表</div>
                <div class="font-medium">{{ detectResult.class_list_ok ? '✓' : '✗' }}</div>
              </Card>
              <Card size="small" :bordered="true" :style="{ borderColor: detectResult.category_ok ? '#52c41a' : '#d9d9d9' }">
                <div class="text-xs text-gray-500">分类</div>
                <div class="font-medium">{{ detectResult.category_ok ? '✓' : '✗' }}</div>
              </Card>
            </div>
          </template>

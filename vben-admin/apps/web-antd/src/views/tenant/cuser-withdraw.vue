...
import { getTenantCUserWithdrawRequestsApi, reviewTenantCUserWithdrawApi, type TenantCUserWithdrawItem } from '#/api/tenant';

const loading = ref(false);
const reviewing = ref(false);
const list = ref<TenantCUserWithdrawItem[]>([]);
const pagination = reactive({ page: 1, limit: 20, total: 0 });
const filterCUID = ref('');
const filterStatus = ref('');
const reviewVisible = ref(false);
const reviewStatus = ref<1 | -1>(1);
const reviewRemark = ref('');
const currentRow = ref<TenantCUserWithdrawItem | null>(null);

const columns = [
  { title: 'ID', dataIndex: 'id', key: 'id', width: 70 },
  { title: '会员ID', dataIndex: 'c_uid', key: 'c_uid', width: 90 },
  { title: '会员', key: 'member', ellipsis: true },
...
async function loadData(page = 1) {
  ...
  try {
    const res = await getTenantCUserWithdrawRequestsApi({
      page: pagination.page,
      limit: pagination.limit,
      c_uid: filterCUID.value || undefined,
      status: filterStatus.value || undefined,
    });
    list.value = res.list || [];
    pagination.total = res.pagination?.total || 0;
  } catch (e: any) {
    message.error(e?.message || '加载会员提现申请失败');
  } finally {
    loading.value = false;
  }
}
...
function openReview(row: TenantCUserWithdrawItem, status: 1 | -1) {
...
async function submitReview() {
  ...
  try {
    await reviewTenantCUserWithdrawApi(currentRow.value.id, {
...
<template>
  <Page title="会员提现" content-class="p-4">
    <Card>
      <div class="mb-4 flex flex-wrap items-center gap-3">
        <Space wrap>
          <Input v-model:value="filterCUID" placeholder="会员ID" allow-clear style="max-width: 120px; min-width: 90px" @pressEnter="() => loadData(1)" />
      ...
      <Spin :spinning="loading">
        <Table :data-source="list" :columns="columns" :pagination="false" row-key="id" size="small" bordered :scroll="{ x: 1240 }">
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'member'">
              <div class="text-xs leading-5">
                <div>{{ record.nickname || '-' }}</div>
                <div class="text-gray-400">{{ record.account || '-' }}</div>
              </div>
            </template>
            ...
            <template v-else-if="column.key === 'action'">
              <Space v-if="record.status === 0">
                <Button type="link" size="small" @click="openReview(record, 1)">确认已打款</Button>
        ...
        <div class="mt-4 flex justify-end" v-if="pagination.total > pagination.limit">
          <Pagination :current="pagination.page" :total="pagination.total" :page-size="pagination.limit" show-size-changer @change="(p: number) => loadData(p)" />
        </div>
      </Spin>
    </Card>

    <Modal v-model:open="reviewVisible" :title="reviewStatus === 1 ? '确认已线下打款' : '驳回会员提现申请'" :confirm-loading="reviewing" ok-text="提交" @ok="submitReview">
      <div v-if="currentRow" class="space-y-4">
        ...
        <Input.TextArea v-model:value="reviewRemark" :rows="4" :placeholder="reviewStatus === 1 ? '可填写线下转账备注' : '请填写驳回原因'" />
      </div>
    </Modal>
  </Page>
</template>

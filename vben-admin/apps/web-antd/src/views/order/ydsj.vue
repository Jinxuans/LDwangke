...
import {
  ...
  Select, SelectOption, Form, FormItem,
} from 'ant-design-vue';
import type { YDSJOrder } from '#/api/ydsj';
import {
  ydsjOrderListApi,
  ydsjAddOrderApi,
  ydsjRefundOrderApi,
...
function getStatusText(s: number) {
  const map: Record<number, string> = { 1: '等待处理', 2: '处理成功', 3: '处理失败', 4: '退款成功' };
  return map[s] || `${s}`;
}
function getStatusColor(s: number) {
  const map: Record<number, string> = { 1: 'warning', 2: 'success', 3: 'error', 4: 'default' };
...
const columns = [
  ...
  { title: '里程', dataIndex: 'distance', width: 70 },
...
<template>
  <Page title="运动世界" description="管理运动世界/小步点跑步订单">
    ...
    <Card :bordered="false">
      ...
      }" row-key="id" :scroll="{ x: 1300 }" size="small">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'account'">
            <div class="leading-tight">{{ record.user }}</div>
            <div class="text-gray-400 text-xs">{{ record.pass }}</div>
          </template>
          <template v-if="column.key === 'run_type'">{{ getRunTypeText(record.run_type) }}</template>
          ...
          <template v-if="column.key === 'is_run'">
          ...
          <template v-if="column.key === 'action'">
            <Space>
              ...
              <Button size="small" danger @click="handleRefund(record)" :disabled="record.status === 4">退款</Button>
            </Space>
          </template>
        </template>
      </Table>
    ...
    <Modal v-model:open="addVisible" title="添加订单" width="550px" :confirm-loading="addLoading" @ok="handleAdd">
      <Form layout="horizontal" :label-col="{ span: 5 }">
        ...
        <FormItem label="学校"><Input v-model:value="addForm.school" placeholder="请输入学校" /></FormItem>
        ...
        <FormItem label="总公里数"><Input v-model:value="addForm.distance" placeholder="请输入总公里数" /></FormItem>
        <FormItem label="开始小时"><InputNumber v-model:value="addForm.start_hour" :min="0" :max="23" style="width: 100%" /></FormItem>
        <FormItem label="开始分钟"><InputNumber v-model:value="addForm.start_minute" :min="0" :max="59" style="width: 100%" /></FormItem>
        <FormItem label="结束小时"><InputNumber v-model:value="addForm.end_hour" :min="0" :max="23" style="width: 100%" /></FormItem>
        <FormItem label="结束分钟"><InputNumber v-model:value="addForm.end_minute" :min="0" :max="59" style="width: 100%" /></FormItem>
        <FormItem label="跑步周期">

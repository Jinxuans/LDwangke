...
function setupAccessGuard(router: Router) {
  router.beforeEach(async (to, from) => {
    ...
    try {
      ...
      // 自定义字体
      if (siteConfig?.fontsZDY === '1' && siteConfig?.fontsFamily) {
        document.documentElement.style.fontFamily = siteConfig.fontsFamily;
      }
      // 维护模式：非管理员强制退出
      if (siteConfig?.bz === '1' && !userRoles.includes('admin')) {
        accessStore.setAccessToken(null);
        return {
          path: LOGIN_PATH,
          query: { msg: 'maintenance' },
          replace: true,
        };
      }
    } catch { /* ignore */ }

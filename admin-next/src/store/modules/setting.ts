...
const VISUAL_BASELINE_STORAGE_KEY = 'sys-visual-baseline'
const VISUAL_BASELINE_VERSION = '2026-04-14-art-baseline-v1'
const TAB_STYLE_OPTIONS = ['tab-default', 'tab-card', 'tab-google'] as const
...
export const useSettingStore = defineStore(
  ...
  () => {
    ...
    const normalizePersistedVisualSettings = () => {
      const menuTypeOptions = Object.values(MenuTypeEnum)
      const menuThemeOptions = Object.values(MenuThemeEnum)
      const containerWidthOptions = Object.values(ContainerWidthEnum)

      if (!menuTypeOptions.includes(menuType.value)) {
        menuType.value = SETTING_DEFAULT_CONFIG.menuType
      }

      if (!menuThemeOptions.includes(menuThemeType.value)) {
        menuThemeType.value = SETTING_DEFAULT_CONFIG.menuThemeType
      }

      if (!TAB_STYLE_OPTIONS.includes(tabStyle.value as (typeof TAB_STYLE_OPTIONS)[number])) {
        tabStyle.value = SETTING_DEFAULT_CONFIG.tabStyle
      }

      if (!containerWidthOptions.includes(containerWidth.value)) {
        containerWidth.value = SETTING_DEFAULT_CONFIG.containerWidth
      }

      const radius = Number(customRadius.value)
      if (!Number.isFinite(radius) || radius <= 0 || radius > 2) {
        customRadius.value = SETTING_DEFAULT_CONFIG.customRadius
      }
    }

    const applyVisualBaseline = (force = false) => {
      normalizePersistedVisualSettings()

      if (typeof window === 'undefined') {
        return
      }

      const storedBaselineVersion = localStorage.getItem(VISUAL_BASELINE_STORAGE_KEY)
      const shouldApplyBaseline = force || storedBaselineVersion !== VISUAL_BASELINE_VERSION

      if (shouldApplyBaseline) {
        menuType.value = SETTING_DEFAULT_CONFIG.menuType
        menuOpenWidth.value = SETTING_DEFAULT_CONFIG.menuOpenWidth
        menuOpen.value = SETTING_DEFAULT_CONFIG.menuOpen
        dualMenuShowText.value = SETTING_DEFAULT_CONFIG.dualMenuShowText
        menuThemeType.value = SETTING_DEFAULT_CONFIG.menuThemeType
        showMenuButton.value = SETTING_DEFAULT_CONFIG.showMenuButton
        showFastEnter.value = SETTING_DEFAULT_CONFIG.showFastEnter
        showRefreshButton.value = SETTING_DEFAULT_CONFIG.showRefreshButton
        showCrumbs.value = SETTING_DEFAULT_CONFIG.showCrumbs
        showWorkTab.value = SETTING_DEFAULT_CONFIG.showWorkTab
        boxBorderMode.value = SETTING_DEFAULT_CONFIG.boxBorderMode
        tabStyle.value = SETTING_DEFAULT_CONFIG.tabStyle
        customRadius.value = SETTING_DEFAULT_CONFIG.customRadius
        containerWidth.value = SETTING_DEFAULT_CONFIG.containerWidth

        localStorage.setItem(VISUAL_BASELINE_STORAGE_KEY, VISUAL_BASELINE_VERSION)
      }

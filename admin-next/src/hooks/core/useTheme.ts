...
let stopPreferredDarkWatch: (() => void) | null = null

const applyHtmlThemeClass = (theme: SystemThemeEnum) => {
  const el = document.documentElement
  const themeClasses = Object.values(AppConfig.systemThemeStyles)
  ...
  if (currentTheme?.className) {
    el.classList.add(currentTheme.className)
  }
}
...
export function initializeTheme() {
  ...
  const applyThemeByMode = () => {
    ...
    // 设置主题 class
    applyHtmlThemeClass(actualTheme)
  ...
  stopPreferredDarkWatch?.()
  stopPreferredDarkWatch = watch(
    prefersDark,
    () => {

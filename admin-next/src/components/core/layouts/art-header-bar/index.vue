...
<template>
  ...
  >
    ...
    <ArtNotification
      ...
      :notice-list="notificationNoticeList"
      :msg-list="notificationMessageList"
      :pending-list="[]"
...
<script setup lang="ts">
  ...
  const { unreadChatCount, notificationNoticeList, notificationMessageList } =
    storeToRefs(inboxStore)

  const showNotice = ref(false)
  const hasUnreadChat = computed(() => unreadChatCount.value > 0)
  ...
  onMounted(() => {
    initLanguage()
    inboxStore.loadInboxData()
    document.addEventListener('click', bodyCloseNotice)
    inboxTimer = setInterval(() => {
      inboxStore.loadInboxData({ silent: true })
    }, 30000)
  })
   ...
   * 打开聊天窗口
   */
  const openChat = async (sessionId?: number): Promise<void> => {
    await inboxStore.openPreferredSession(sessionId)
    mittBus.emit('openChat', inboxStore.activeSessionId)
  }
  ...
  const handleMessageClick = async (id: number | string): Promise<void> => {
    showNotice.value = false
    await openChat(Number(id))
  }

  const handleViewAll = async (tab: 'notice' | 'message' | 'pending'): Promise<void> => {
    if (tab === 'message') {
      await openChat()
    }
  }
